...
bool IMUFactor::Evaluate(double const* const* parameters, double* residuals, double** jacobians) const {
	...
	if (jacobians) {
		// the differential of the pre-integration end time error relative to the pre-integration start time error is
		// used to correct the pre-integration increment
		...
		if (pre_integration->jacobian.maxCoeff() > 1e8 || pre_integration->jacobian.minCoeff() < -1e8) {
			LOGGER_WARN("numerical unstable in preintegration");
		}
...
void MarginalizationInfo::preMarginalize() {
	...
	for (auto it : factors) {
		...
		for (int i = 0; i < static_cast<int>(block_sizes.size()); i++) {
			...
			// copy the parameter block in factor to parameter_block_data, parameter_block_data is the data of the
			// entire optimization variable
...
void MarginalizationInfo::marginalize() {
	...
	if (m == 0) {
		valid = false;
		LOGGER_WARN("no variable to be marginalized, unstable tracking...");
		return;
	}
	...
	for (int j = 0; j < NUM_THREADS; j++) {
		threads_struct[j].A = Eigen::MatrixXd::Zero(pos, pos);
		threads_struct[j].b = Eigen::VectorXd::Zero(pos);
...
ProjectionOneFrameTwoCamFactor::ProjectionOneFrameTwoCamFactor(Eigen::Vector3d _pts_i, Eigen::Vector3d _pts_j,
                                                               const Eigen::Vector2d& _velocity_i,
                                                               const Eigen::Vector2d& _velocity_j, const double _td_i,
                                                               const double _td_j)
    : pts_i(std::move(_pts_i))
    , pts_j(std::move(_pts_j))
    , td_i(_td_i)
    , td_j(_td_j) {
...
ProjectionTwoFrameOneCamFactor::ProjectionTwoFrameOneCamFactor(Eigen::Vector3d _pts_i, Eigen::Vector3d _pts_j,
                                                               const Eigen::Vector2d& _velocity_i,
                                                               const Eigen::Vector2d& _velocity_j, const double _td_i,
                                                               const double _td_j)
    : pts_i(std::move(_pts_i))
...
ProjectionTwoFrameTwoCamFactor::ProjectionTwoFrameTwoCamFactor(Eigen::Vector3d _pts_i, Eigen::Vector3d _pts_j,
                                                               const Eigen::Vector2d& _velocity_i,
                                                               const Eigen::Vector2d& _velocity_j, const double _td_i,
                                                               const double _td_j)
    : pts_i(std::move(_pts_i))
