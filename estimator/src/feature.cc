 ...
 */
template <typename T> static void reduceVector(std::vector<T>& v, std::vector<uchar> status) {
	int j = 0;
...
void FeatureTracker::setMask() {
	...
	// traverse feature point, re-input to point vector
...
void FeatureTracker::setSemanticMask(const cv::Mat& semantic) { semantic_img = semantic; }

bool FeatureTracker::inSemantic(const cv::Point& p) {
	if (!USE_SEGMENTATION || semantic_img.empty())
		return false;
...
void FeatureTracker::readIntrinsicParameter(const std::vector<std::string>& calib_file) {
	for (const auto& i : calib_file) {
		LOGGER_INFO("reading parameter of camera: ", i.c_str());
		CameraModel::CameraPtr _camera = CameraModel::CameraFactory::instance()->generateCameraFromYamlFile(i);
	...
	if (DEPTH)
		use_rgbd_cam = true;
	LOGGER_INFO("USE STEREO: ", use_stereo_cam, " USE RGB-D: ", use_rgbd_cam);
}
...
FeatureFrame FeatureTracker::trackImage(double _cur_time, const cv::Mat& img_0, const cv::Mat& img_1) {
	...
	if (!prev_pts.empty()) {
		...
		if (!USE_GPU_ACC) {
			...
			// if it has predicted in estimator before
			if (has_prediction) {
				cur_pts = predict_pts;
			...
			if (FLOW_BACK) {
				...
				// both the forward and reverse optical flow are matched and the distance from the original point is not
				// more than 0.5 pixels
			...
			if (has_prediction) {
				...
				if (succ_num < 10) {
					...
					std::vector<cv::Point2f> tmp1_cur_pts(cur_gpu_pts.cols);
					cur_gpu_pts.download(tmp1_cur_pts);
					cur_pts = tmp1_cur_pts;

					std::vector<uchar> tmp1_status(gpu_status.cols);
			...
			if (FLOW_BACK) {
				...
				std::vector<cv::Point2f> reverse_pts(reverse_gpu_pts.cols);
...
#endif
		LOGGER_DEBUG("cur_pts size: ", cur_pts.size());
		...
		LOGGER_DEBUG("after reduce cur_pts size: ", cur_pts.size());
	}
	...
	LOGGER_DEBUG("n_max_cnt: ", n_max_cnt);
	if (!USE_GPU_ACC) {
		if (n_max_cnt > 0) {
			if (mask.empty())
				LOGGER_INFO("mask is empty ");
			if (mask.type() != CV_8UC1)
				LOGGER_INFO("mask type wrong ");
			// extract corner points
			cv::goodFeaturesToTrack(cur_img, n_pts, MAX_CNT - static_cast<int>(cur_pts.size()), 0.01, MIN_DIST, mask);
		}
	...
	if (!img_1.empty() && use_stereo_cam) {
		...
		if (!cur_pts.empty()) {
			...
			if (!USE_GPU_ACC) {
				std::vector<float> err;
				// cur left ---- cur right
				cv::calcOpticalFlowPyrLK(cur_img, right_img, cur_pts, cur_right_pts, status, err, cv::Size(21, 21), 3);
				// reverse check cur right ---- cur left
				if (FLOW_BACK) {
					cv::calcOpticalFlowPyrLK(right_img, cur_img, cur_right_pts, reverseLeftPts, statusRightLeft, err,
					                         cv::Size(21, 21), 3);
					for (auto i = 0; i < status.size(); i++) {
				...
				std::vector<cv::Point2f> tmp_cur_right_pts(cur_right_gpu_pts.cols);
				...
				if (FLOW_BACK) {
					...
					std::vector<cv::Point2f> tmp_reverseLeft_Pts(reverseLeft_gpu_Pts.cols);
					reverseLeft_gpu_Pts.download(tmp_reverseLeft_Pts);
					reverseLeftPts = tmp_reverseLeft_Pts;

					std::vector<uchar> tmp1_status(status_gpu_RightLeft.cols);
	...
	// total_cost += t_feature_teacking.toc();

	// LOGGER_WARN("gpu mode: ", USE_GPU_ACC ? "ON" : "OFF");
	// LOGGER_WARN("feature tracking cost: ", t_feature_teacking.toc());
	// LOGGER_WARN("average tracking cost: ", total_cost / count);
...
std::vector<cv::Point2f> FeatureTracker::undistortedPts(std::vector<cv::Point2f>& pts,
	...
	for (auto& pt : pts) {
		// feature point pixel coordinate
...
std::vector<cv::Point2f> FeatureTracker::ptsVelocity(std::vector<int>& ids_, std::vector<cv::Point2f>& pts,
...
void FeatureTracker::rejectWithF() {
	if (cur_pts.size() >= 8) {
		...
		for (unsigned int i = 0; i < cur_pts.size(); i++) {
			...
			camera[0]->liftProjective(Eigen::Vector2d(cur_pts[i].x, cur_pts[i].y), tmp_p);
			tmp_p.x() = FOCAL_LENGTH * tmp_p.x() / tmp_p.z() + col / 2.0;
			tmp_p.y() = FOCAL_LENGTH * tmp_p.y() / tmp_p.z() + row / 2.0;
			un_cur_pts[i] = cv::Point2f((float)tmp_p.x(), (float)tmp_p.y());

			camera[0]->liftProjective(Eigen::Vector2d(prev_pts[i].x, prev_pts[i].y), tmp_p);
			tmp_p.x() = FOCAL_LENGTH * tmp_p.x() / tmp_p.z() + col / 2.0;
			tmp_p.y() = FOCAL_LENGTH * tmp_p.y() / tmp_p.z() + row / 2.0;
...
void FeatureTracker::setPrediction(std::map<int, Eigen::Vector3d>& _predict_pts) {
	has_prediction = true;
	predict_pts.clear();
	std::map<int, Eigen::Vector3d>::iterator predict_it;
...
void FeatureTracker::drawTrack(const cv::Mat& img_left, const cv::Mat& img_right, std::vector<int>& _cur_left_ids,
	...
	// draw a circle for the feature points in the left picture, the number of red tracking is less, and the number of
	// blue tracking is more
	for (auto j = 0; j < _cur_left_pts.size(); j++) {
		...
		if (status == FeatureLevel::DYNAMIC) {
			...
			// cv::putText(img_track, to_string(ids[j]), _cur_left_pts[j], cv::FONT_HERSHEY_SIMPLEX, 0.45, cv::Scalar(0,
			// 255, 255), 1.8);
			...
			// cv::putText(img_track, to_string(ids[j]), _cur_left_pts[j], cv::FONT_HERSHEY_SIMPLEX, 0.45,
			// cv::Scalar(255 * (1 - len), 0, 255 * len), 1.8);
		}
	}
	...
	// draw an arrow for feature points in the left image and aim to the position of feature points in the previous
	// frame
	...
	for (auto i = 0; i < _cur_left_ids.size(); i++) {
		...
		if (map_it != _prev_left_pts_map.end()) {
			cv::arrowedLine(img_track, _cur_left_pts[i], map_it->second, cv::Scalar(0, 255, 0), 1, 8, 0, 0.2);
		}
	}
}

cv::Mat FeatureTracker::getTrackImage() { return img_track; }
...
std::map<int, int> FeatureTracker::getFeatureStatus() { return pts_status; }

int FeaturePerId::endFrame() const { return start_frame + static_cast<int>(feature_per_frame.size()) - 1; }
...
void FeatureManager::setDepthImage(const cv::Mat& depth) { depth_img = depth; }

bool FeatureManager::addFeatureCheckParallax(int frame_count, FeatureFrame& image, double td) {
	LOGGER_DEBUG("input feature: ", image.size());
	LOGGER_DEBUG("num of feature: ", getFeatureCount());
	...
	if (frame_count < 2 || last_track_num < 20 || long_track_num < 200 || new_feature_num > 0.5 * last_track_num)
		return true;
	...
	for (auto& _it : feature) {
		...
		if (it_per_id.start_frame <= frame_count - 2 &&
			...
			// calculate the distance of the current feature point on the normalized camera plane in the first two
			// frames
	...
	if (parallax_num == 0) {
		...
		// check whether the disparity between the first two frames is large enough (10 pixels in the pixel coordinate
		// system)
		LOGGER_DEBUG("parallax_sum: ", parallax_sum, ", parallax_num: ", parallax_num);
		LOGGER_DEBUG("current parallax: ", parallax_sum / parallax_num * FOCAL_LENGTH);
...
void FeatureManager::setDepth(const std::map<int, double>& deps) {
	for (auto& it : deps) {
		int _id = it.first;
...
std::map<int, double> FeatureManager::getDepthVector() {
	// This function gives actually points for solving; We only use the oldest max_solve_cnt point, oldest pts has good
	...
	for (auto& _it : feature) {
		...
		if (it_per_id.used_num >= 2 && it_per_id.start_frame < WINDOW_SIZE - 2) {
			total_feature_num++;
			if (pts_status[it_per_id.feature_id] == FeatureLevel::DYNAMIC)
				dynamic_feature_num++;
		}
	}
	LOGGER_INFO("total feature num: ", total_feature_num);
	LOGGER_INFO("dynamic feature num: ", dynamic_feature_num);
	LOGGER_INFO("ratio: ", (double)dynamic_feature_num / total_feature_num);
	stable_scale = 1.5 * (total_feature_num - dynamic_feature_num) / total_feature_num;
...
bool FeatureManager::solvePoseByPnP(Eigen::Matrix3d& R, Eigen::Vector3d& P, std::vector<cv::Point2f>& pts2D,
	...
	if (int(pts2D.size()) < 4) {
		LOGGER_WARN("feature tracking not enough, please slowly move you device!");
		return false;
	}
...
void FeatureManager::initFramePoseByPnP(int frame_cnt, Eigen::Vector3d Ps_[], Eigen::Matrix3d Rs_[],
                                        Eigen::Vector3d tic_[], Eigen::Matrix3d ric_[]) {
	if (frame_cnt > 0) {
		...
		for (auto& it : feature) {
			...
			if (it_per_id.estimated_depth > 0) {
				...
				if ((int)it_per_id.feature_per_frame.size() >= index + 1) {
					...
					Eigen::Vector3d pts_in_body =
					    ric_[0] * (it_per_id.feature_per_frame[0].point * it_per_id.estimated_depth) + tic_[0];
					// convert to world coordinate from the first frame IMU coordinate
					Eigen::Vector3d pts_in_world =
					    Rs_[it_per_id.start_frame] * pts_in_body + Ps_[it_per_id.start_frame];

					cv::Point3f point3d(static_cast<float>(pts_in_world.x()), static_cast<float>(pts_in_world.y()),
		...
		if (solvePoseByPnP(R_camera, P_camera, pts2D, pts3D)) {
			...
			Eigen::Quaterniond Q(Rs_[frame_cnt]);
		}
	}
}

void FeatureManager::triangulate(Eigen::Vector3d Ps_[], Eigen::Matrix3d Rs_[], Eigen::Vector3d tic_[],
                                 Eigen::Matrix3d ric_[]) {
	// traverse feature points in the current frame
	for (auto& it : feature) {
		auto& it_per_id = it.second;
		...
		if (DEPTH) {
			...
			// tr: Pwc   Rr: Rwc
			Eigen::Vector3d tr = Ps_[imu_i] + Rs_[imu_i] * tic_[0];
			Eigen::Matrix3d Rr = Rs_[imu_i] * ric_[0];
			...
			// and average the depth on the initial frame of the point whose re-projection error is less than the
			// threshold as the estimated depth
			for (int k = 0; k < (int)it_per_id.feature_per_frame.size(); k++) {
				if (it_per_id.feature_per_frame[k].depth == 0) {
					no_depth_num++;
					continue;
				}
				Eigen::Vector3d t0 = Ps_[imu_i + k] + Rs_[imu_i + k] * tic_[0];
				Eigen::Matrix3d R0 = Rs_[imu_i + k] * ric_[0];
				// point0: Pc'
				...
				for (int j = 0; j < (int)it_per_id.feature_per_frame.size(); j++) {
					if (k == j)
						continue;
					Eigen::Vector3d t1 = Ps_[imu_i + j] + Rs_[imu_i + j] * tic_[0];
					Eigen::Matrix3d R1 = Rs_[imu_i + j] * ric_[0];
					// t20: tc'c"  R20: Rc'c"
					...
					Eigen::Vector2d residual = point1_2d - Eigen::Vector2d(point1_projected.x() / point1_projected.z(),
			...
			if (verified_depths.empty()) {
				if (rough_depths.empty()) {
					if (no_depth_num == it_per_id.feature_per_frame.size()) {
						...
						Eigen::Vector3d t0 = Ps_[imu_j] + Rs_[imu_j] * tic_[0];
						Eigen::Matrix3d R0 = Rs_[imu_j] * ric_[0];
						...
						Eigen::Vector3d t1 = Ps_[imu_j] + Rs_[imu_j] * tic_[0];
		...
		// if it is stereo camera or monocular camera
			...
			if (STEREO && it_per_id.feature_per_frame[0].is_stereo) {
				...
				int imu_i = it_per_id.start_frame;
				Eigen::Matrix<double, 3, 4> left_pose;
				Eigen::Vector3d t0 = Ps_[imu_i] + Rs_[imu_i] * tic_[0];
				...
				Eigen::Matrix<double, 3, 4> right_pose;
				Eigen::Vector3d t1 = Ps_[imu_i] + Rs_[imu_i] * tic_[1];
				Eigen::Matrix3d R1 = Rs_[imu_i] * ric_[1];
				right_pose.leftCols<3>() = R1.transpose();
				right_pose.rightCols<1>() = -R1.transpose() * t1;
			...
			// for monocular triangulation, the observation frame must be at least 2 frames
				...
				imu_i++;
				Eigen::Matrix<double, 3, 4> right_pose;
				Eigen::Vector3d t1 = Ps_[imu_i] + Rs_[imu_i] * tic_[0];
				Eigen::Matrix3d R1 = Rs_[imu_i] * ric_[0];
				right_pose.leftCols<3>() = R1.transpose();
				right_pose.rightCols<1>() = -R1.transpose() * t1;
	...
	for (auto _it = feature.begin(), it_next = feature.begin(); _it != feature.end(); _it = it_next) {
		...
		if (it.start_frame != 0)
			...
			if (it.feature_per_frame.size() < 2) {
				...
				// estimated_depth is the depth value under the observation frame of the first frame, and now it is
				// updated to the next frame
