...
bool GlobalSFM::solveFrameByPnP(Eigen::Matrix3d& R_initial, Eigen::Vector3d& P_initial, int i,
	...
	for (auto j = 0; j < m_feature_num; j++) {
		...
		for (auto k = 0; k < sfm_feature[j].observation.size(); k++) {
			...
			if (sfm_feature[j].observation[k].first == i) {
				...
				Eigen::Vector2d img_pts = sfm_feature[j].observation[k].second;
	...
	if (pts_2_vector.size() < 15) {
		LOGGER_WARN("unstable features tracking, please slowly move you device!");
...
bool GlobalSFM::construct(int frame_num, Eigen::Quaterniond* Q, Eigen::Vector3d* T, int l,
	...
	// initial two view, set frame l to reference coordinate (world coordinate), Q[l] | T[l]: c0 frame, Q[frame-1] |
	// T[frame-1]: R(c0<--ck), P(c0<--ck)
	Q[l].setIdentity();
	T[l].setZero();
	// the current frame
	...
	Eigen::Matrix3d camera_rotation[frame_num];
	Eigen::Vector3d camera_translation[frame_num];
	Eigen::Quaterniond camera_quaternion[frame_num];
	...
	for (auto i = l; i < frame_num - 1; i++) {
		...
		if (i > l) {
			...
			Eigen::Matrix3d R_initial = camera_rotation[i - 1];
			Eigen::Vector3d P_initial = camera_translation[i - 1];
	...
	for (auto i = 0; i < m_feature_num; i++) {
		...
		for (auto j = 0; j < sfm_feature[i].observation.size(); j++) {
			...
			ceres::CostFunction* cost_function = ReprojectionError3D::Create(sfm_feature[i].observation[j].second.x(),
			                                                                 sfm_feature[i].observation[j].second.y());

			// residual: R(c-->c0), t(c-->c0), 3d feature point in world (c0) coordinate
			problem.AddResidualBlock(cost_function, nullptr, camera_rotation_ba[k], camera_translation_ba[k],
			                         sfm_feature[i].position);
		}
	}
	...
	for (auto i = 0; i < frame_num; i++) {
		// T(ci<--c0) --> T(c0<--ci)
		T[i] = -1 * (Q[i] * Eigen::Vector3d(camera_translation_ba[i][0], camera_translation_ba[i][1],
		                                    camera_translation_ba[i][2]));
	}
	// save sfm track point (after ceres optimization), 3d point in frame l (c0) coordinate
	for (auto& feature : sfm_feature) {
		if (feature.state)
			sfm_tracked_points[feature.id] =
			    Eigen::Vector3d(feature.position[0], feature.position[1], feature.position[2]);
	}
	return true;
}
...
void solveGyroscopeBias(std::map<double, ImageFrame>& all_image_frame, Eigen::Vector3d* Bgs) {
	...
	LOGGER_INFO("gyroscope bias initial calibration ", delta_bg.transpose());
...
Eigen::MatrixXd tangentBasis(Eigen::Vector3d& g0) {
	...
	if (a == tmp)
		tmp << 1, 0, 0;
	// schimidt orthogonalization to get vector b and calculate c through a X b to get a linearly independent set of
...
bool linearAlignment(std::map<double, ImageFrame>& all_image_frame, Eigen::Vector3d& g, Eigen::VectorXd& x) {
	...
	for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++) {
		...
		// convert to positive definite so that could solve problem by cholesky decomposition
		Eigen::MatrixXd r_A = tmp_A.transpose() * cov_inv * tmp_A; // r_A(10 * 10)
		Eigen::VectorXd r_b = tmp_A.transpose() * cov_inv * tmp_b; // r_b(10 * 1)
	...
	LOGGER_WARN(" refine     ", g.norm(), " ", g.transpose());

	// get scale factor
	if (!DEPTH) {
		double s = (x.tail<1>())(0) / 100.0;
		LOGGER_INFO("estimated scale: %f", s);
		(x.tail<1>())(0) = s;
		if (s < 0.0)
			return false;
		else
