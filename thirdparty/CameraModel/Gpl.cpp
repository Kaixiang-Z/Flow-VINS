...
void LLtoUTM(double latitude, double longitude, double &utmNorthing,
    ...
    utmNorthing =
        k0 * (M + N * tan(LatRad) * (A * A / 2.0 + (5.0 - T + 9.0 * C + 4.0 * C * C) * A * A * A * A / 24.0 + (61.0 - 58.0 * T + T * T + 600.0 * C - 330.0 * eccPrimeSquared) * A * A * A * A * A * A / 720.0));
    if (latitude < 0.0) {
...
void UTMtoLL(double utmNorthing, double utmEasting, const std::string &utmZone,
    ...
    LongOrigin =
        (ZoneNumber - 1.0) * 6.0 - 180.0 + 3.0; //+3 puts origin in middle of zone
