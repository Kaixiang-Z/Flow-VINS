...
// ---------------------------------------------IMU Residual--------------------------------------------- //
    ...
    bool IMUFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
        ...
        // information matrix
        Matrix<double, 15, 15> sqrt_info = LLT<Matrix<double, 15, 15>>(
 ...
 */
    ...
    bool ProjectionOneFrameTwoCamFactor::Evaluate(double const *const *parameters,
        ...
        if (jacobians) {
            ...
            reduce << 1. / dep_j, 0, -pts_camera_j(0) / (dep_j * dep_j), 0, 1. / dep_j, -pts_camera_j(1) /
                                                                                        (dep_j * dep_j);
...
// ---------------------------------------------Marginalization Residual--------------------------------------------- //
    ...
    void MarginalizationInfo::marginalize() {
        ...
        // get inverse of Amm
        MatrixXd Amm_inv =
                saes.eigenvectors() * VectorXd(
                        (saes.eigenvalues().array() > eps).select(saes.eigenvalues().array().inverse(),
                                                                  0)).asDiagonal() * saes.eigenvectors().transpose();
    ...
    bool MarginalizationFactor::Evaluate(double const *const *parameters,
        ...
        for (int i = 0; i < static_cast<int>(marginalization_info->keep_block_size.size()); i++) {
            ...
            if (size != 7)
                ...
                dx.segment<3>(idx + 3) =
                        2.0 *
                        (Quaterniond(x0(6), x0(3), x0(4), x0(5)).inverse() * Quaterniond(x(6), x(3), x(4), x(5))).vec();
                if ((Quaterniond(x0(6), x0(3), x0(4), x0(5)).inverse() * Quaterniond(x(6), x(3), x(4), x(5))).w() < 0) {
                    dx.segment<3>(idx + 3) = 2.0 * -(Quaterniond(x0(6), x0(3), x0(4), x0(5)).inverse() *
                                                     Quaterniond(x(6), x(3), x(4), x(5))).vec();
                }
            }
        }
        // update residual, r = r0 + J0*dx taylor expansion
        Map<VectorXd>(residuals, n) =
