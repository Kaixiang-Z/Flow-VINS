...
void YOLOv8_seg::inferenceProcess() {
    while (true) {
        double time = 0;
        cv::Mat img, mask;
        ...
        if (!img.empty()) {
            TicToc t_semantic;
            objs.clear();
            copyFromMat(img, size);
            inference();
            postProcess(objs);
            drawObjects(img, mask, objs);
            ROS_INFO_STREAM("semantic inference cost : " << t_semantic.toc() << "ms");
 ...
 */
bool YOLOv8_seg::semanticAvailable(double t) {
    if (!mask_buf.empty()) {
        if (t == mask_buf.front()->header.stamp.toSec()) {
            return true;
        } else {
            mutex_segment.lock();
            mask_buf.pop();
...
void YOLOv8_seg::copyFromMat(const cv::Mat &image, cv::Size &size) {
    ...
    CHECK_CUDA(cudaMemcpyAsync(this->device_ptrs[0],
                               nchw.ptr<float>(),
...
void YOLOv8_seg::inference() {
    ...
    for (int i = 0; i < this->num_outputs; i++) {
        ...
        CHECK_CUDA(cudaMemcpyAsync(this->host_ptrs[i],
