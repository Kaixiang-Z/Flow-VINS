...
namespace FLOW_VINS {
    ...
    Estimator::Estimator() :
            init_thread_flag(false), eskf_estimator() {
    ...
    void Estimator::setParameter() {
        ...
        cout << "MULTIPLE_THREAD is " << MULTIPLE_THREAD << '\n';
        if (MULTIPLE_THREAD && !init_thread_flag) {
            init_thread_flag = true;
            thread_estimator = thread(&Estimator::processMeasurements, this);
            thread_loopfusion = thread(&Estimator::processLoopFusion, this);
        }
    }
    ...
    void Estimator::inputAhrs(double t, const Vector3d &linear_acceleration, const Vector3d &angular_velocity,
    ...
    void Estimator::inputImage(double t, const cv::Mat &_img,
        ...
        mutex_loopfusion.lock();
        loop_image_buf.push(make_pair(t, _img));
        mutex_loopfusion.unlock();
    ...
    void Estimator::inputReloFrame(double frame_stamp, int frame_index, vector<Vector3d> &match_points,
        ...
        prev_relo_t = move(relo_t);
        prev_relo_r = move(relo_r);
    ...
    void Estimator::processMeasurements() {
        while (true) {
            ...
            vector<pair<double, Vector3d>> acc_vector, gyr_vector, mag_vector;
            ...
            if (!feature_buf.empty()) {
                ...
                if (USE_IMU) {
                    ...
                    for (auto i = 0; i < n; ++i) {
                        ...
                        Vector3d tmp_g = acc_vector[n - var_size + i].second / var_dt;
                        sum_g += tmp_g;

                        // use ESKF to estimator pose
                        if (USE_MAGNETOMETER) {
                            Matrix<double, 10, 1> measurement;
                            measurement
                                    << acc_vector[i].second, gyr_vector[i].second, mag_vector[i].second, acc_vector[i].first;
                            Quaterniond q = eskf_estimator.run(measurement);
                            q.normalize();
                    ...
                    for (auto i = n - var_size; i < n; i++) {
                        ...
                        Vector3d tmp_g = acc_vector[i].second / dt;
                        var += (tmp_g - aver_g).transpose() * (tmp_g - aver_g);
                    }
                    // calculate the standard deviation of the IMU acceleration
                    var = sqrt(var / (n - 1));
                    //ROS_WARN("Imu Variance: %lf", var);
                }
    ...
    void Estimator::initFirstImuPose(vector<pair<double, Vector3d>> &acc_vector) {
        cout << "init first imu pose" << endl;
        ...
        cout << "averge acc" << average_acc.transpose() << endl;
    ...
    void Estimator::initFirstAhrsPose(vector<pair<double, Vector3d>> &acc_vector,
                                      vector<pair<double, Vector3d>> &mag_vector) {
        cout << "init first ahrs pose" << endl;
        init_first_pose_flag = true;
        // calculate average acceleration
        Vector3d average_acc(0, 0, 0);
        int n = (int) acc_vector.size();
    ...
    bool Estimator::getImuInterval(
            double t0, double t1, vector<pair<double, Vector3d>> &acc_vector,
            vector<pair<double, Vector3d>> &gyr_vector) {
        if (acc_buf.empty()) {
            cout << "not receive imu" << endl;
            return false;
        }
    ...
    bool Estimator::getAhrsInterval(
            double t0, double t1, vector<pair<double, Vector3d>> &acc_vector,
            vector<pair<double, Vector3d>> &gyr_vector, vector<pair<double, Vector3d>> &mag_vector) {
        if (acc_buf.empty()) {
            cout << "not receive ahrs" << endl;
            return false;
        }
        // extract the data of the time period (t0, t1)
        if (t1 <= acc_buf.back().first) {
    ...
    void Estimator::buildSfmFeature(vector<SFMFeature> &sfm_feature) {
    ...
    bool Estimator::solveRelativeRT(const vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &rotation,
    ...
    bool Estimator::solvePnPForAllFrame(Quaterniond Q[], Vector3d T[], map<int, Vector3d> &sfm_tracked_points) {
        map<double, ImageFrame>::iterator frame_it;
    ...
    void Estimator::nonLinearOptimization(ceres::Problem &problem, ceres::LossFunction *loss_function) {
        ...
        if (USE_IMU) {
            for (int i = 0; i < frame_count; i++) {
                ...
                problem.AddResidualBlock(imu_factor, nullptr, para_pose[i], para_speed_bias[i], para_pose[j],
                                         para_speed_bias[j]);
            }
        }
    ...
    void Estimator::double2vector() {
        ...
        if (USE_IMU) {
            ...
            if (relocalization_info) {
                Matrix3d relo_r;
                Vector3d relo_t;
                relo_r = Quaterniond(relo_pose[6], relo_pose[3], relo_pose[4],
                                     relo_pose[5]).normalized().toRotationMatrix();
    ...
    void Estimator::margNew() {
        ...
        if (last_marginalization_info &&
            count(begin(last_marginalization_parameter_blocks), end(last_marginalization_parameter_blocks),
    ...
    void Estimator::movingConsistencyCheck(set<int> &remove_index) {
        ...
        if (solver_flag == Estimator::SolverFlag::NON_LINEAR && marginalization_flag == MARGIN_OLD) {
            ...
            for (auto &it: feature_manager.feature) {
                ...
                if (it_per_id.start_frame < WINDOW_SIZE - 2 &&
                    it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2 && it_per_id.solve_flag == NON_LINEAR) {
                    ...
                    point_cloud.cam_points.emplace_back(cv::Point2f(it_per_id.feature_per_frame[imu_j].point.x(),
                                                                    it_per_id.feature_per_frame[imu_j].point.y()));
                    point_cloud.uv_points.emplace_back(cv::Point2f(it_per_id.feature_per_frame[imu_j].uv.x(),
            ...
            mutex_loopfusion.lock();
            loop_pose_buf.push(make_pair(headers[WINDOW_SIZE - 2], Pose));
            loop_point_buf.push(make_pair(headers[WINDOW_SIZE - 2], point_cloud));
            LOOP_TIC = tic[0];
            LOOP_QIC = ric[0];
            mutex_loopfusion.unlock();
        }
        ...
        while (true) {
            ...
            if (!loop_image_buf.empty() && !loop_point_buf.empty() && !loop_pose_buf.empty()) {
                if (loop_image_buf.front().first > loop_pose_buf.front().first) {
                    loop_pose_buf.pop();
                    cout << "throw pose at beginning" << endl;
                } else if (loop_image_buf.front().first > loop_point_buf.front().first) {
                    loop_point_buf.pop();
                    cout << "throw point at beginning" << endl;
            ...
            if (!image.empty()) {
                ...
                auto *keyframe = new KeyFrame(time, frame_index, T, R,
                                              image,
                                              point_cloud.world_points, point_cloud.uv_points, point_cloud.cam_points,
